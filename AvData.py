...
class AvData:
    ...
    def updateAptData(self):
        self.__update_data()
        full_path = os.path.realpath(__file__)
        path,fn=os.path.split(full_path)
        filename = path+"/data/APT.txt"
        self.apts = self.__parseAirports(filename)
        shutil.rmtree(path+"/data")
        pass

    def getData(self,icao_ident):
        if not self.apts:
            self.updateAptData()
        myAptIdent = icao_ident
        myApt = None
        for a in self.apts:
            if a.icao_ident==myAptIdent:
                ...
                break
        if not myApt:
            return "No airport with ID found."
        return myApt

    def nearestApts(self,icao_ident,num=10,min_rwy_length=8000,string=True,getCenters=True):
        if not self.apts:
            self.updateAptData()
        myAptIdent = icao_ident
        myApt = None
        ...
        for a in self.apts:
            ...
            if l>=min_rwy_length:
                nearestApts.append([a,dist])

        nearestApts_sorted = sorted(nearestApts, key=lambda x: x[1])
        nearestApts_sorted=nearestApts_sorted[:min(num,len(nearestApts_sorted))]
        centers = []
        if getCenters:
            for a in nearestApts_sorted:
                if not a[0].responsible_artcc in centers:
                    centers.append(a[0].responsible_artcc)
        if string:
            nearestAptStr = []
            for a in nearestApts_sorted:
                nearestAptStr.append(str(a[0]))
            nearestAptStr.extend(centers)
            return nearestAptStr
        else:
            nearestAptObj = []
            for a in nearestApts_sorted:
                nearestAptObj.append(a[0])
            nearestAptObj.extend(centers)
            return nearestAptObj
    ...
    def getAHASRisk(self,icao,month,day,hour_zulu,all_data=False):
        print("Downloading AHAS data...")
        ...
        if not icao in ahas_keys:
            return "No AHAS data available."
        past_str = ""
        if datetime.utcnow()>datetime(year,month,day,hour_zulu,0,0,0):
            past_str = "_PAST"
            url = "https://www.usahas.com/webservices/Fluffy_AHAS.asmx/GetAHASRisk"+past_str+"?Area=%27"+ahas_keys[icao]+"%27&iMonth="+str(month)+"&iDay="+str(day)+"&STime="+str(hour_zulu)+":00"
        else:
            url = "https://www.usahas.com/webservices/Fluffy_AHAS.asmx/GetAHASRisk"+past_str+"?Area=%27"+ahas_keys[icao]+"%27&iMonth="+str(month)+"&iDay="+str(day)+"&iHour="+str(hour_zulu)
        session = HTMLSession()
        ...
        for table in tables:
            ...
            dateTime = table.find("DateTime").get_text()
            nexradRisk = table.find("NEXRADRISK").get_text()
            soarRisk = table.find("SOARRISK").get_text()
            ahasRisk = table.find("AHASRISK").get_text()
            basedOn = table.find("NEXRADRISK").get_text()
            TIDepth = table.find("TIDepth").get_text()
            ahas_data.append([route,segment,hour,dateTime,nexradRisk,soarRisk,ahasRisk,basedOn,TIDepth])

        if not all_data:
            return ahas_data[0][6]
        return ahas_data

    def getPRAIM(self,baro_aid=True):
        ...
        enroute_url = "https://sapt.faa.gov/enroute"+baro_str+".png?t="+str(enroute_num)
        terminal_url = "https://sapt.faa.gov/terminal"+baro_str+".png?t="+str(terminal_num)
        npa_url = "https://sapt.faa.gov/npa"+baro_str+".png?t="+str(npa_num)
    ...
    def bearingRange(self,icao_ident_origin,icao_ident_dest):
        origin = self.getData(icao_ident_origin)
        destination = self.getData(icao_ident_dest)
        g = Geod(ellps='WGS84')
        fwd_az,back_az,dist = g.inv(origin.location[1],origin.location[0],destination.location[1],destination.location[0])
        if fwd_az<0:
            fwd_az+=360
        dist = dist/1852
        return [fwd_az,dist]
    ...
    def __update_data(self):
        ...
        full_path = os.path.realpath(__file__)
        path,filename=os.path.split(full_path)
        self.__download_url(self.__generateURL(),path+'/temp.zip')
        shutil.unpack_archive(path+'/temp.zip',path+'/data')
        os.remove(path+'/temp.zip')

    def __parseAirports(self,filename):
        ...
        for i in range(len(lines)):
            if lines[i][0:3]=='APT':
                ...
                apt.location = self.__convertToDeg([lines[i][523:537],lines[i][550:565]])
    ...
    def __convertToDeg(self,coords):
        s1 = coords[0]
        s2 = coords[1]
...
ad = AvData()
ad.getPRAIM(False)
# l = ad.nearestApts("KVPS")
# kvps = ad.getData("KVPS")
# print(ad.bearingRange("KVPS","KATL"))
